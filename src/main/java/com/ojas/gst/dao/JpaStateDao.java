...
public class JpaStateDao extends JpaDao<State, Long> implements StateDao {
	...
	public int deleteById(State state) {
		Query query = getEntityManager().createNativeQuery(
				"update state set state_status='" + state.getStatus() + "' where state_id=" + state.getId());
		return query.executeUpdate();
	}

	@Override
	public int updateStateById(State state) {
		Query query = getEntityManager().createNativeQuery("update state set state_status='" + state.getStatus()
				+ "',state_name ='" + state.getStateName() + "',state_updatedBy ='" + state.getLastUpdatedBy()
				+ "'taxpayerstype_updatedDate'" + state.getUpdatedDate() + "'where state_id=" + state.getId());
		return query.executeUpdate();
	}
